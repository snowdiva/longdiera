...
class BrowserConsoleHandler extends AbstractProcessingHandler
{
    ...
    {
        return new LineFormatter('[[%channel%]]{macro: autolabel} [[%level_name%]]{web-weight: bold} %message%');
    }
    ...
    {
        $args = array(self::quote('web-weight: normal'));
        $format = '%c' . $formatted;
        preg_match_all('/\[\[(.*?)\]\]\{([^}]*)\}/s', $format, $matches, PREG_OFFSET_CAPTURE | PREG_SET_ORDER);

        foreach (array_reverse($matches) as $match) {
            $args[] = self::quote(self::handleCustomStyles($match[2][0], $match[1][0]));
            $args[] = '"web-weight: normal"';
    ...
    private static function dump($title, array $dict)
    {
        ...
        $script[] = self::call('log', self::quote('%c%s'), self::quote('web-weight: bold'), self::quote($title));
